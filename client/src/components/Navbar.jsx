...
function Navbar(props) {
  ...
  return (
    <nav className={`fixed top-0 w-full px-4 py-2 md:px-12 md:py-3 bg-red-600 text-white font-sans flex items-center z-10 transition-transform duration-300 ${hidden ? '-translate-y-full' : 'translate-y-0'} `}>
      <div className="logo flex gap-3 items-center cursor-pointer">
        <svg
          fill="#fb8500"
        ...
        <span className="font-bold">BlogIt</span>
      </div>
      
      {props.sideElements == true ? (
        <div className="buttons ml-auto flex items-center">
        <Link to="/signin" className="px-4 py-2 text-[14px] transition-all duration-300 hover:text-black">SIGN IN</Link>
        <div className='hidden md:block'><BlogBtn /></div>
      </div>
      ) : null}
    </nav>
  );
